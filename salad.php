...
function getSaladsFromDatabase($db) {
    $salads = [];
    $stmt = $db->query("SELECT * FROM restaurantMenuv2 WHERE Type = 'Salad'");
...
<body>
    ...
    <?php
    $salads = getSaladsFromDatabase($db);

    foreach ($salads as $salad) {
        echo '<button class="menu-button" onclick="showModifications(\'' . $salad['Name'] . '\')">' . $salad['Name'] . '</button>';
    }
    ?>
