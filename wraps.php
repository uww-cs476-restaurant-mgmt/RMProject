...
function getWrapsFromDatabase($db) {
    $wraps = [];
    $stmt = $db->query("SELECT * FROM restaurantMenuv2 WHERE Type = 'Wrap'");
...
<body>
    ...
    <?php
    $wraps = getWrapsFromDatabase($db);

    foreach ($wraps as $wrap) {
        echo '<button class="menu-button" onclick="showModifications(\'' . $wrap['Name'] . '\')">' . $wrap['Name'] . '</button>';
    }
    ?>
    <script>
